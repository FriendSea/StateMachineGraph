...
public class RigidbodyMovement : MonoBehaviour, FriendSea.StateMachine.Behaviours.IMovement, FriendSea.StateMachine.Conditions.IPlatformerObject
{
    [SerializeField]
    ...
    Rigidbody _target;
    Rigidbody Target => _target ?? (_target = GetComponent<Rigidbody>());
    ...
    {
        var delta = Target.position + Velocity * Time.fixedDeltaTime;
        var groundDist = CheckGround();
        if (!float.IsNaN(groundDist))
            delta += -transform.up * groundDist;
    ...
    float CheckGround()
    {
        ...
        var count = Physics.RaycastNonAlloc(from, (to - from), results, (to - from).magnitude);
